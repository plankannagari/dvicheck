...
## Migrations
- `V1`–`V6` exist. **`V7`** = `add_budget_goal` (`budget_amount` on `users`, Day 20). **`V8`** = `add_item_feedback` (`item_feedback` table, Day 23). **`V9`** = `add_budget_alert_log` (`budget_alert_log` table, Day 24). **`V10`** = `add_onboarding_completed` (`onboarding_completed` on `users`, Day 25). Next free version is **`V11`**.
