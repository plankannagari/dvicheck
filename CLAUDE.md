...
- **`ShoppingListScreen` and `PantryScreen` don't exist** (flagged repeatedly through Day 18): the real shopping screen is `ListsScreen.js` (Day 9 naming); there is no Pantry screen at all — Pantry is backend-only (`PantryService`/`PantryMemory`), never had a mobile UI. Only `HomeScreen`, `InsightsScreen`, and `ListsScreen`'s detail view got this pattern; Pantry was skipped entirely, nothing to wire

## Migrations
- `V1`–`V6` exist (`V6` = `push_token`). Next free version is **`V7`** — no migration was needed for Day 16, 17, or 18 (all response-DTO/service-layer/mobile-only changes, no schema change).
