...
## Deep linking (established Day 27)
- No Flyway migration today — `V11` is still the next free version; mobile-only day, backend untouched
- `app.json`: `"scheme": "dvicheck"` added (didn't exist before) — `dvicheck://` is now a real, working prefix, not aspirational
- `mobile/src/utils/deepLinking.js` — `setPendingDeepLink`/`consumePendingDeepLink` (a simple module-level singleton; one pending URL at a time is enough for this app's scope), `parseDeepLink(url)` (via `Linking.parse`, supports `dvicheck://bill/{id}` and `dvicheck://insights`, `null` otherwise — includes a defensive fallback branch plus a `__DEV__`-only log of the parsed shape, since expo-linking's exact hostname/path split for a custom scheme was asserted, not empirically verified, at write time), internal `processDeepLink(url)`, and exported `handleIncomingUrl(url, isAuthenticated)` / `consumeAndProcessPendingDeepLink()`
- **Deliberately no `linking` prop on `NavigationContainer`.** That prop lets React Navigation restore nav state directly from a URL, which would bypass the Day 25 auth/onboarding gate entirely — an incoming link could push a user straight to a screen without ever going through `SplashScreen`'s `isAuthenticated`/`onboardingCompleted` check. Instead: `Linking.getInitialURL()` (cold start) and `Linking.addEventListener('url', ...)` (warm start), both in `App.js`, only ever feed the pending-link singleton — never navigate directly. The pending link is only consumed after `SplashScreen` or `OnboardingScreen` has confirmed auth + onboarding state, via the same `setTimeout(fn, 0)` deferred-tick pattern in both places (`navigationRef`'s ready state lags one tick behind a `replace()` call that was just dispatched)
- Bill deep links call `useHistoryStore.getState().loadBillDetail(id)` directly, then `navigate('MainApp', { screen: 'History' })` — there is no separate `BillDetail` route; `HistoryScreen`'s detail view has always been store-state-driven (keyed off `activeBill`, Day 10), so setting that state is sufficient to land on the right screen showing the right bill
- `loadBillDetail` catches its own errors internally and never rejects (sets `error` in `historyStore` state, doesn't throw) — so failure detection in `deepLinking.js` checks `useHistoryStore.getState().error` immediately after `await`ing the call, not `.catch()`. On failure (bad id, wrong owner, network error), shows a toast via `useToastStore.getState().showToast(...)` — reuses `GET /api/bills/{id}`'s existing Day 10 ownership check as-is, no backend change needed, so a link to someone else's bill fails the same way the endpoint already handles it
- `insights` deep links call `navigate('Insights')` — **not** `InsightsDetail`, which has never existed at any point in this app (see the repeated corrections in Days 23/24's sections above)
- Share: `HistoryScreen`'s detail-view header now has a Share button (replacing what was actually `<View style={styles.backBtn} />`, a 32px spacer — not the `{width:72}` spacer the task description assumed). `handleShare` builds the link via `Linking.createURL('bill/' + activeBill.id)` — never hardcode `'dvicheck://'` directly, since `createURL` resolves to the right scheme/host automatically in both Expo Go/dev-client and standalone builds. **Known cosmetic side effect**: the new `shareBtn` style is `width: 72` while the back button stays `width: 32`, so the header is no longer symmetric and the title isn't perfectly centered — implemented with the exact widths given rather than silently forcing symmetry, but worth a follow-up if it looks off in practice
- React Native's `Share.share({message, url, ...})` only honors the separate `url` field on iOS; Android requires the link to already be inline in `message` to be shared at all — `handleShare`'s message text includes the link for this reason, `url` is not Android's actual delivery mechanism
- **Scope**: custom scheme (`dvicheck://`) only. HTTPS universal links (tapping a real `https://` link opens the app) need a hosted `apple-app-site-association` file on a real domain — deferred until after the Day 39 production deploy, since there's no production domain to host it on yet

## Offline mode (established Day 28)
- No Flyway migration today — `V11` is still the next free version; mobile-only day, backend untouched (other than one client-side timeout fix — see below)
- **Package reality check before writing any code**: `expo-file-system` did **not** actually resolve for app code despite being expected "to ship with Expo by default" — it existed only nested inside `expo`'s own `node_modules` (not hoisted, not in `package.json`), confirmed via a failed `require.resolve`. Ran `npx expo install expo-file-system` to fix. Separately, `@react-native-async-storage/async-storage` and `@react-native-community/netinfo` both turned up already present in `package.json` mid-session, without this session installing either — noted, not investigated further, since both resolved correctly once checked
- **SDK 54's `expo-file-system` gotcha**: the package's default export (`import ... from 'expo-file-system'`) is a *new* `File`/`Directory`-class API — it has no `documentDirectory`, `getInfoAsync`, `makeDirectoryAsync`, `copyAsync`, or `deleteAsync`. The classic imperative API this app's offline code needs only exists under **`expo-file-system/legacy`**. Verified directly against the installed package's type declarations before writing `syncQueue.js`, rather than assuming the old API still worked at the top-level import
- Offline scope is deliberately **capture-and-queue, not real offline processing** — OCR/Gemini/Claude all require connectivity, so an offline capture just saves the photo (`expo-file-system/legacy`'s `documentDirectory`, survives an app restart) and queues metadata; the actual scan/categorisation only ever happens once back online
- `networkStore.js`: `isConnected` (optimistic `true` default), `initialized`. `state.isInternetReachable === null` (still being determined, common right after the listener starts) is treated as reachable — only an explicit `false` counts as offline, avoiding a false "offline" flash on startup. Owns its own `NetInfo.addEventListener` subscription and triggers `processQueue()` itself on a `false→true` transition — no other file watches for reconnection. `initNetworkListener()` is idempotent (guarded by a module-level `unsubscribe` variable) and called once at module scope in `App.js`, same "runs immediately on JS load" pattern as Day 27's cold-start deep link capture
- `syncQueue.js`: `QUEUE_KEY`/`PENDING_DIR` constants; `getQueue()` never throws (returns `[]` + `console.warn` on any read/parse failure); `addToQueue()` copies (not just references) the capture into `PENDING_DIR` under a timestamp+random id — copying is what survives a restart, since camera/picker temp URIs aren't guaranteed to persist; `removeFromQueue()` filters+persists first, then deletes the local file with `idempotent: true` plus its own try/catch, so a missing file on cleanup is never a failure. `processQueue()`: module-level `isProcessing` flag guards a manual "Sync now" tap and an automatic reconnect trigger from ever running concurrently; iterates the queue **sequentially** (not `Promise.all`), to avoid hammering the scan endpoint with simultaneous multipart uploads right after a connection recovers; on a per-item failure, re-reads the queue before persisting the incremented `retryCount`/`lastError` (rather than reusing its own loop-start snapshot) — `addToQueue()` could have appended a new item concurrently while that item was uploading, and writing back a stale snapshot would silently drop it; after the loop, a `synced > 0` fires both a toast and a fully-silent `loadDashboard()` refresh
- `ScanScreen.js`: new `'queued'` phase alongside `camera`/`processing`/`result`. The offline branch sits at the very top of the existing `handleScan(uri)` — still the one shared function for both capture and gallery-pick, confirmed unchanged since Day 8 before this work started — so neither call site needed touching. Camera phase shows a small amber banner when `!isConnected`, so the offline/queued behavior isn't a surprise mid-capture
- `HomeScreen.js`: "Sync now" banner (shown when `pendingCount > 0`, refreshed via `useFocusEffect` on every tab focus — not just mount, since the queue can change from `ScanScreen` or a background auto-sync while Home wasn't focused) is separate from the offline-cache banner (shown when `isOfflineCache` is true) — two different banners, two different concerns (pending uploads vs. stale displayed data)
- Cached reads (`homeStore`, `historyStore`): the cache is only ever read/hydrated **on a genuine fetch failure**, never speculatively read first — `isOfflineCache` flips `true` only inside the `catch` block, and back to `false`/`cachedAt: null` on the next real success. `historyStore` only caches the `refresh`/page-0 view (index view); pagination failures keep their original generic-error behavior unchanged, since there's no cache for pages beyond the first. One cross-step gap caught and fixed while wiring the offline-banner UI: `historyStore`'s hydration branch was originally only setting `isOfflineCache: true` with no timestamp, leaving `HistoryScreen` nothing to render a relative time from — added `cachedAt` to its state (initial value, hydration, and reset-on-success) to match `homeStore`'s shape
- `formatCacheTime()` (in both `HomeScreen.js` and `HistoryScreen.js`, duplicated rather than shared — matching this codebase's established per-screen-helper convention, e.g. `fmt`/`fmtDate`): minute buckets → hour buckets → `toLocaleTimeString()` fallback for anything a day+ old. No relative-time library added — none existed in the codebase already, and one wasn't worth adding just for this
- **Real bug found and fixed via live device testing, not part of the original Day 28 plan**: the very first queued item to actually retry failed with `timeout of 10000ms exceeded`, working exactly as designed (left queued, `retryCount` incremented) — but the underlying cause was a genuine pre-existing gap, not an offline-code bug. `POST /api/bills/scan` runs Vision OCR + Gemini extraction + Gemini/Claude categorisation sequentially server-side (~7s typical, confirmed via a timed direct curl), leaving almost no margin under `apiClient.js`'s flat 10s global timeout — a slightly-slower network right after reconnecting from offline was enough to trip it client-side even though the backend would have finished. Fixed with a **per-request** timeout override (`timeout: 30000`) on just the `/bills/scan` call in `scanApi.js`'s `uploadReceiptImage`, leaving the global 10s default intact for every other (genuinely fast) endpoint — deliberately not a global timeout increase, so a truly hung request elsewhere still fails fast

## Performance & caching (established Day 29)
- `V11__add_performance_indexes.sql`: `CREATE INDEX IF NOT EXISTS idx_bills_user_purchase_date ON bills(user_id, purchase_date)` (genuinely new — no composite existed on `bills` before this; covers `sumTotalBetween`/`sumTotalSince`/`sumAvoidableSince`/`countBillsSince`/`findRecentByUserId`/`sumTotalsGroupedByWeek`, all of which filter on both columns together) and a second `CREATE INDEX IF NOT EXISTS idx_line_items_bill_id ON line_items(bill_id)` — this one is a **no-op**, since `idx_line_items_bill_id` already existed from `V2__create_core_tables.sql`; confirmed via psql (`\d line_items`) before writing the migration, not assumed
- Postgres does **not** automatically index foreign-key columns the way some other databases do — worth checking explicitly (`\di`) on any new FK before assuming it's covered, rather than trusting this specific instance where `line_items.bill_id` already happened to have one
- `GET /api/bills` N+1 fix: `BillHistoryController.getBills()` restructured so both the search and non-search branches resolve to a common `List<Bill> bills` first, then one bulk `BillRepository.countLineItemsByBillIds(billIds)` query builds a `Map<UUID, Long>`, then `toRecentBillDto(bill, itemCounts)` reads `itemCounts.getOrDefault(bill.getId(), 0L)` instead of the old `bill.getLineItems().size()` lazy load — 1-plus-N queries collapsed to exactly 2 per request regardless of page size. General pattern for this kind of fix: bulk-fetch an id→value map before a `.stream().map()` loop, rather than touching a lazy collection per entity
- `SpendingTrendsService.getTrends()`: the old 8-iteration loop (one `sumTotalBetween` call per week) replaced by one native query, `BillRepository.sumTotalsGroupedByWeek(userId, earliestWeekStart)` (`date_trunc('week', purchase_date)`), grouped into a `Map<LocalDate, BigDecimal>` that the same 8-week loop then reads from. The query casts to `::date` on top of the plain `date_trunc` result — Postgres returns `timestamptz` (not `date`) when truncating a `date` column by week (confirmed via `pg_typeof`), which would otherwise leave the JDBC→`Object[]` row mapping ambiguous across driver/Hibernate versions. Verified Monday-aligned truncation directly via `SELECT date_trunc('week', CURRENT_DATE)` before trusting it
- `SpendingTrendsService.getTrends()` **is** cached — `CacheConfig` (new, `config/`) defines a Caffeine-backed `spendingTrends` cache (`expireAfterWrite(5, MINUTES)`, `maximumSize(1000)`), `@Cacheable(value = "spendingTrends", key = "#userId")` on `getTrends()`. A companion `evictTrendsCache(userId)` (`@CacheEvict`, empty body) is called from `BillScanController.scanBill()` right alongside the existing Day 24 `budgetAlertService.checkAndSendAlerts(currentUserId())` call, so a freshly scanned bill shows up in the trends chart immediately instead of waiting out the 5-minute TTL
- `MonthlyReportService.getMonthlyReport()` is deliberately **NOT** cached — confirmed (grep, not assumed) it has exactly two Java callers: `MonthlyReportController`'s `GET /api/reports/monthly` handler and `BudgetAlertService.checkAndSendAlerts()` (Day 24), which needs current numbers immediately after every bill save to correctly detect an 80%/100% threshold crossing. Caching this method would need a rethink of that freshness guarantee first — do not add `@Cacheable` here casually
- Mobile: `HomeScreen`/`HistoryScreen` converted from a single destructured `useXStore()` call to one `useXStore((s) => s.field)` selector per field actually referenced in the component — avoids re-rendering on unrelated store-field changes. Action functions (`loadDashboard`, `loadBills`, etc.) are stable Zustand references, selected the same way, no `useCallback` needed. Not yet applied to any other screen
- `HistoryScreen`'s index-view `FlatList` (bills list only, not the detail-view line-items list) tuned with `removeClippedSubviews={true}`, `initialNumToRender={10}`, `maxToRenderPerBatch={10}`, `windowSize={7}` — rendering-only props, no change to `onEndReached`/`keyExtractor`/`renderItem`
- `homeStore.loadDashboard()` and `historyStore.loadBills()` both gained a re-entrancy guard (`if (get().isLoading) return;` at the very top, before the existing `set(...)` /success/error logic) — relevant since Day 28's `useFocusEffect` on `HomeScreen` can trigger `processQueue()` → a background `loadDashboard()` call that could otherwise overlap with an in-flight manual pull-to-refresh

## Backend patterns (established Day 30)
- `app.auth.dev-mode` (`application.yml`, env var `AUTH_DEV_MODE`, defaults `true`) — **must be set to `false`** before any real external testing (TestFlight Day 42, production Day 39). Defaults `true` for local dev specifically to avoid burning real Twilio SMS credits (via Supabase) on every backend restart
- Supabase is used **only** for OTP delivery/verification (`SupabaseAuthClient`, wired into `AuthService` behind the `dev-mode` flag) — dvicheck still mints its own JWT afterward via the unchanged `JwtService`. Do not confuse Supabase's own session tokens with dvicheck's auth tokens; the app never sees or stores a Supabase token
- `PATCH /api/bills/{billId}/items` (`BillHistoryController.addManualItems` → `BillService.addManualItems`) — manually entered items always get category `ESSENTIAL` and `confidence 1.0` (human-entered, not a model guess); response reuses the exact same `toDetailResponse` mapping `GET /api/bills/{billId}` uses (moved from a private controller method to a public `BillService` method so both call sites share one implementation, no duplicate mapping logic)
- Code review pass (Days 22-29, done Day 30): one critical fix applied — `BillHistoryController.addManualItems()` was missing the method-level `@Transactional` override needed to escape the controller's class-level `readOnly = true` (the same override `updateBill()` already had, with a comment claiming it was "the one write in this controller" — no longer true once `addManualItems()` was added). Without it, the write would silently join the existing read-only transaction and fail against Postgres. Moderate/minor findings deferred — see Tech debt below

## Migrations
- `V1`–`V6` exist. **`V7`** = `add_budget_goal` (`budget_amount` on `users`, Day 20). **`V8`** = `add_item_feedback` (`item_feedback` table, Day 23). **`V9`** = `add_budget_alert_log` (`budget_alert_log` table, Day 24). **`V10`** = `add_onboarding_completed` (`onboarding_completed` on `users`, Day 25). **`V11`** = `add_performance_indexes` (`bills(user_id, purchase_date)` composite — new; `line_items(bill_id)` — pre-existing no-op, see Day 29 section above) — still the last migration; Day 30's fixes were Java-only, no schema change needed. Next free version is **`V12`**.
...
From Days 22-29 code review (Day 30) — critical fix applied (see Backend patterns above); these moderate/minor findings deferred:
- `BillScanController.scanBill()`: `spendingTrendsService.evictTrendsCache(...)` (Day 29) has no try/catch of its own, unlike the self-contained `budgetAlertService.checkAndSendAlerts(...)` right before it — if cache eviction ever throws, it would propagate out of `scanBill()` and turn an already-successful bill save into a 500 response to the client
- `historyStore.loadBillDetail()` hardcodes `error: 'Could not load this bill.'` instead of the `error.appError?.message || fallback` convention its siblings (`loadBills`, `editBill`) already follow — matters more since Day 27's `deepLinking.js` depends on this store's `error` state to detect a failed bill deep link, and now gets a generic message regardless of the real cause (404 vs 403 vs network)
- `OTPVerifyScreen.js` and `PhoneEntryScreen.js` (both touched Day 25) still read `err.response?.data?.message` instead of `err.appError?.message` — the same bug class already found and fixed in `ScanScreen.js` during Day 23, missed in these two auth screens
- `ScanScreen.handlePickFromGallery()` has no try/catch (unlike `handleCapture()` beside it) — an unhandled rejection with no user-facing feedback if gallery permission/picker calls throw
- Hardcoded values worth eventually promoting to config: `BudgetAlertService.THRESHOLDS` (80/100, Day 24), `CacheConfig`'s `expireAfterWrite(5, MINUTES)` / `maximumSize(1000)` (Day 29)
- `currentUserId()` duplication is now in more controllers than the "9 as of Day 23" figure above reflects (also `MonthlyReportController`, Day 22's `/trends` endpoint) — that count is stale
- `CacheConfig`'s `CaffeineCacheManager("spendingTrends")` is constructed with one hardcoded cache name — won't auto-resolve a second `@Cacheable` cache name if one's ever added without updating this constructor
...
✅ Day 27 complete — `dvicheck://` custom scheme, `deepLinking.js` utility (pending-link singleton + parse/process), `App.js` cold/warm URL capture, `SplashScreen`/`OnboardingScreen` consume pending links post-auth, Share button on bill detail via `Linking.createURL()`
✅ Day 28 complete — offline scan capture + sync queue (`syncQueue.js`, `networkStore.js`), auto-sync on reconnect + manual "Sync now" banner, cached reads for Home/History with an offline banner — plus a real scan-timeout bug found and fixed via live device testing (see Day 28 section above)
✅ Day 29 complete — Flyway `V11` (`bills` user+date composite index — new; `line_items` FK index — confirmed pre-existing from V2, no-op), `GET /api/bills` N+1 fix (bulk count query), `SpendingTrendsService` 8-query loop collapsed to 1 grouped query + 5-min Caffeine cache with eviction on new bill save, Zustand selector conversion on Home/History, `FlatList` tuning, `loadDashboard`/`loadBills` re-entrancy guards
✅ Day 30 complete — real Supabase phone OTP wired in (`app.auth.dev-mode` toggle for local work), manual item entry now persists via `PATCH /api/bills/{id}/items`, code review pass on Days 22-29 with the one critical finding fixed (moderate/minor findings tracked as tech debt — see above)
🔄 Day 31 next — RevenueCat monetisation integration
