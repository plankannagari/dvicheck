...
## Migrations
- `V1`–`V6` exist. **`V7`** = `add_budget_goal` (`budget_amount` on `users`, Day 20). Next free version is **`V8`** — untouched Days 21 and 22.
...
- `currentUserId()` is copy-pasted into **8** controllers as of Day 21 (`ShoppingListController`, `BillHistoryController`, `InsightsController`, `UserController`, `BillScanController`, `HomeController`, `PantryController`, `MonthlyReportController`) — up from 5 at the time this note was first written (Day 13); behaviorally identical today, but drift-prone and growing
...
🔄 Day 23 next — Item-level feedback (thumbs up/down on scan suggestions), feedback stored in DB
